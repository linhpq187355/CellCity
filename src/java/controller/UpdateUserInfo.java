...
public class UpdateUserInfo extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if(ud.searchByUName(username)!=null && !username.equals(a.getUsername())){
            request.setAttribute("er", "Tên tài khoản đã tồn tại! Thử lại");
            request.getRequestDispatcher("updateInfo.jsp").forward(request, response);
        } else {
            User k = new User(a.getUserId(),name,email,username,"",address,phone,0,0,null);
            ud.updateInfo(k);
            User u = ud.searchByUName(k.getUsername());
