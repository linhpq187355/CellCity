        ...
        <script type="text/javascript">
            function doDelete(id) {
                if (confirm("Bạn có chắc muốn xóa sản phẩm này không?")) {
                    window.location = "delproitem?id=" + id;
                }
            }
        </script>
    ...
    <body>
        <div class="wrapper">
                    ...
                    <div class="page-inner">
                        <div class="page-header">
                            ...
                            <ul class="breadcrumbs mb-3">
                                ...
                                <li class="nav-item">
                                    <a href="listproadmin?id=0">Tất cả sản phẩm và phiên bản</a>
                                </li>
                            </ul>
                        </div>
                        <div class="row">
            ...
            // Kiểm tra nếu có thông báo thành công
                                                <% if (session.getAttribute("successMessageUpdate") != null) { %>
            alert('<%= session.getAttribute("successMessageUpdate") %>');
                                                <% session.removeAttribute("successMessageUpdate"); %>
                                                <% } else if (session.getAttribute("errorMessageUpdate") != null) { %>
            alert('<%= session.getAttribute("errorMessageUpdate") %>');
                                                <% session.removeAttribute("errorMessageUpdate"); %>
                                                <% } %>
            ...
            alert('<%= session.getAttribute("errorMessageDelete") %>');
                                                ...
                                                <% } %>
                                                <% if (session.getAttribute("successMessageSale") != null) { %>
            alert('<%= session.getAttribute("successMessageSale") %>');
                                                <% session.removeAttribute("successMessageSale"); %>
                                                <% } else if (session.getAttribute("errorMessageSale") != null) { %>
            alert('<%= session.getAttribute("errorMessageSale") %>');
                                                <% session.removeAttribute("errorMessageSale"); %>
                                                <% } %>
                                            </script>
                                            <table
                                                id="add-row"
                                                ...
                                                <tbody>
                                                    <c:forEach items="${requestScope.listpd}" var="lpd">

                                                        <tr data-url="detailproadmin?id=${lpd.id}">

                                                            <td>${lpd.id}</td>
                                                            <td>
                                                                <c:forEach items="${requestScope.listpro}" var="lp">
                                                                    <c:if test="${lpd.productID == lp.productId}">
                                                                        ${lp.name}
                                                                    </c:if>
                                                                </c:forEach>
                                                                ${lpd.rom}GB 
                                                                <c:forEach items="${requestScope.listco}" var="lc">
                                                                    <c:if test="${lpd.colorId == lc.id}">
                                                                        ${lc.name}
                                                                    </c:if>
                                                                </c:forEach>
                                                            </td>
                                                    ...
                                                    <td>
                                                        <div class="form-button-action">
                                                            <button
                                                                ...
                                                                >
                                                                <a href="updateprodetail?id=${lpd.id}&pid=${lpd.productID}"><i class="fa fa-edit"></i></a>
                                                            </button>
                                                            <button
                                                                type="button"
                                                                data-bs-toggle="tooltip"
                                                                ...
                                                                >
                                                                <a href="javascript:void(0);" onclick="doDelete('${lpd.id}')"><i class="fa fa-times"></i></a>
                                                            </button>
                                                            <button
                                                                type="button"
                                                                data-bs-toggle="tooltip"
                                                                ...
                                                                data-original-title="Remove"
                                                                >
                                                                <a href="setsale?id=${lpd.id}"><i class="fa fa-bolt"></i></a>
                                                            </button>
                                                        </div>
                                                    </td>
                                                    </a>
