 ...
 * @author ASUS
 */
@WebServlet(name = "AddprodetailServ", urlPatterns = {"/addprodetail"})
public class AddprodetailServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddprodetailServ</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddprodetailServ at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Color> listco = cd.getAll();
        request.setAttribute("listco", listco);
        request.setAttribute("proid", id);
        request.getRequestDispatcher("addprodetail.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ProductDetail pd = new ProductDetail(1, screenTech, ram, rom, frontCam, backCam, pin, os, sim, chipset, proid, screenSize, refreshrate, price, stockQuantity, color, null, null, null, null, 0, 0);
            ProductDetailDAO pdd = new ProductDetailDAO();
            boolean check = pdd.insert(pd);
            if (check) {
                // Thêm sản phẩm thành công
                request.getSession().setAttribute("successMessage", "Thêm sản phẩm thành công!");
            } else {
                // Thêm sản phẩm thất bại
                request.getSession().setAttribute("errorMessage", "Thêm sản phẩm thất bại. Vui lòng thử lại!");
            }
            request.setAttribute("proid", proid);
            request.getRequestDispatcher("adddetailoption.jsp").forward(request, response);
